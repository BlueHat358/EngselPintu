...
from app.service.user_namager import UserManager
...
async def build_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, session: dict):
  ...
  balance_info = get_balance(session['api_key'], session["tokens"]["id_token"])
  ...
  keyboard = [
      ...
      [InlineKeyboardButton(
          "🔥 Beli Paket XUT", callback_data="display_family:08a3b1e6-8e78-4e45-a540-b40f06871cfe:false")],
...
async def show_my_packages(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  active_user = user_manager.get_active_session()
  if not active_user:
  ...
  res = send_api_request(
      api_key=user_manager.api_key, path=path, payload_dict=payload,
...
async def display_packages_from_family(update: Update, context: ContextTypes.DEFAULT_TYPE, family_code: str, is_enterprise: bool, page: int = 1):
  ...
  if cached_packages.get('family_code') != family_code:
    ...
    family_data = get_family(
        api_key=user_manager.api_key, tokens=active_user['tokens'],
...
async def build_account_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, db_user: dict):
  """Membangun menu pengelolaan akun."""
  is_auth = is_authenticated(update)
  if not is_auth:
    chat_id = update.effective_chat.id
  ...
  if not users:
    ...
    for user in users:
      is_active = active_user and user["user_id"] == active_user["user_id"]
      active_marker = " ✅" if is_active else ""
      keyboard.append([InlineKeyboardButton(
        f"{user['username']} ({user['user_id']}){active_marker}", callback_data=f"set_active:{user['user_id']}")])

  keyboard.append([InlineKeyboardButton(
    "➕ Tambah Akun Baru", callback_data="add_account")])
  if active_user:
    keyboard.append([InlineKeyboardButton("❌ Hapus Akun Aktif",
                    callback_data=f"remove_account_confirm:{active_user['user_id']}")])

  if user_manager.get_active_session():
    keyboard.append([InlineKeyboardButton(
      "⬅️ Kembali", callback_data="main_menu")])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
  """Handler untuk command /start yang sudah diperbarui."""
  user = update.effective_user
  chat_id = update.effective_chat.id

  # Cek apakah pengguna ada di DB, jika tidak, buat baru
  db_user = user_manager.get_user(user.id)
  if not db_user:
    db_user = user_manager.create_user(user.id, user.username)
    await context.bot.send_message(chat_id, f"Selamat datang, {user.first_name}! Akun Anda telah dibuat. Silakan tambahkan nomor XL Anda.")
    # Langsung arahkan ke menu akun
    await build_account_menu(update, context, db_user)
    return

  # Cek apakah pengguna punya sesi/nomor aktif
  session = user_manager.get_active_session(user.id)
  if session:
    await context.bot.send_message(chat_id, "Selamat datang kembali!")
    await build_main_menu(update, context, session)
  else:
    await context.bot.send_message(chat_id, "Anda belum memiliki nomor aktif. Silakan pilih dari daftar atau tambahkan nomor baru.")
    await build_account_menu(update, context, db_user)
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  if current_state == "WAITING_FOR_PHONE":
    ...
    context.user_data['login_phone'] = text
    if get_otp(text):
      await update.message.reply_text("OTP terkirim, silakan masukkan kodenya:")
      user_state[chat_id] = "WAITING_FOR_OTP"
    else:
      await update.message.reply_text("Gagal mengirim OTP. Pastikan nomor benar dan coba lagi.")
    ...
    tokens = submit_otp(user_manager.api_key, phone_number, text)

    if tokens:
        # Simpan token ke pengguna yang benar
      user_manager.add_token(user_id, int(phone_number),
                             tokens['refresh_token'])
      await update.message.reply_text("✅ Nomor berhasil ditambahkan!")
      del user_state[chat_id]
...
async def button_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  session = user_manager.get_active_session(user_id)

  if data == "main_menu":
    if session:
      await build_main_menu(update, context, session)
    else:
      await query.message.edit_text("Anda tidak punya nomor aktif. Silakan kelola akun.")
      await build_account_menu(update, context, user_manager.get_user(user_id))
  ...
  # --- Bagian Akun ---
    ...
    if user_manager.set_active_number(user_id, number):
      await query.message.reply_text(f"Akun {number} sekarang aktif.")
      await start(update, context)  # Panggil start untuk refresh menu
    else:
      await query.answer("Gagal mengaktifkan nomor.", show_alert=True)
    ...
    number = int(data.split(":")[1])
    user_manager.remove_token(number)
  ...
  # --- Bagian Bookmark ---
    ...
    if not session:
      await query.answer("Aksi ini memerlukan nomor yang aktif. Silakan pilih dari menu akun.", show_alert=True)
      return
    ...
    elif data.startswith("display_family:"):
