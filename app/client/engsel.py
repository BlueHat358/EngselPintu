...
# DIMODIFIKASI: Impor CRYPTO_API_KEY dari config untuk digunakan di sini
from config import CRYPTO_API_KEY
from app.client.encrypt import encryptsign_xdata, java_like_timestamp, ts_gmt7_without_colon, ax_api_signature, decrypt_xdata, get_x_signature_payment, build_encrypted_field, load_ax_fp, ax_device_id

BASE_API_URL = os.getenv("BASE_API_URL")
BASE_CIAM_URL = os.getenv("BASE_CIAM_URL")
if not BASE_API_URL or not BASE_CIAM_URL:
  raise ValueError(
    "BASE_API_URL or BASE_CIAM_URL environment variable not set")
...
def get_new_token(refresh_token: str) -> str:
  ...
  if "error" in body:
    raise ValueError(
      f"Error in response: {body['error']} - {body.get('error_description', '')}")

  return body

# DIMODIFIKASI: Parameter `api_key` sekarang merujuk ke MYXL_API_KEY
...
def send_api_request(
  ...
  # Panggil `encryptsign_xdata` dengan CRYPTO_API_KEY
  encrypted_payload = encryptsign_xdata(
      crypto_api_key=CRYPTO_API_KEY,  # Menggunakan kunci kripto yang diimpor
      method=method,
      path=path,
      id_token=id_token,
  ...
  headers = {
      ...
      "user-agent": UA,
      "x-api-key": api_key,  # Menggunakan MYXL_API_KEY untuk header ini
  ...
  try:
    # Panggil `decrypt_xdata` dengan CRYPTO_API_KEY
    decrypted_body = decrypt_xdata(CRYPTO_API_KEY, json.loads(resp.text))
    return decrypted_body
  except Exception as e:
    print("[decrypt err]", e)
    return resp.text
# ... (Sisa file `engsel.py` yang memanggil `send_api_request` sekarang akan berfungsi dengan benar) ...
...
def get_families(api_key: str, tokens: dict, package_category_code: str) -> dict:
  ...
  res = send_api_request(api_key, path, payload_dict,
...
def get_package(
  ...
  res = send_api_request(api_key, path, raw_payload,
                         tokens["id_token"], "POST")
...
def send_payment_request(
  ...
  try:
    ...
    # PERBAIKAN: Kembalikan dictionary error yang terstruktur, bukan teks mentah.
    return {
        "status": "FAILED",
        "message": "Gagal mendekripsi respons server pada tahap akhir pembayaran.",
...
def purchase_package(
  ...
  purchase_result = send_payment_request(
    api_key, settlement_payload, tokens["access_token"], tokens["id_token"], token_payment, ts_to_sign, payment_for)
