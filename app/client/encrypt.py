...
# HAPUS: API_KEY = os.getenv("API_KEY") -> Tidak lagi digunakan secara global
...
# DIMODIFIKASI: Menerima crypto_api_key sebagai parameter
...
def ax_api_signature(
  ...
  headers = {
      "Content-Type": "application/json",
      "x-api-key": crypto_api_key,  # Menggunakan kunci yang diteruskan
  }
  ...
  response = requests.request(
    "POST", AX_SIGN_URL, json=request_body, headers=headers, timeout=30)
  if response.status_code == 200:
    return response.json().get("ax_signature")
  else:
    raise Exception(f"Signature generation failed: {response.text}")
...
def encryptsign_xdata(
  ...
  response = requests.request(
    "POST", XDATA_ENCRYPT_SIGN_URL, json=request_body, headers=headers, timeout=30)
...
def decrypt_xdata(
  ...
  if not isinstance(encrypted_payload, dict) or "xdata" not in encrypted_payload or "xtime" not in encrypted_payload:
    raise ValueError(
      "Invalid encrypted data format. Expected a dictionary with 'xdata' and 'xtime' keys.")
  ...
  response = requests.request(
    "POST", XDATA_DECRYPT_URL, json=encrypted_payload, headers=headers, timeout=30)
...
def get_x_signature_payment(
  ...
  response = requests.request(
    "POST", PAYMENT_SIGN_URL, json=request_body, headers=headers, timeout=30)
...
def get_x_signature_bounty(
  ...
  response = requests.request(
    "POST", BOUNTY_SIGN_URL, json=request_body, headers=headers, timeout=30)
  if response.status_code == 200:
    return response.json().get("x_signature")
  else:
    raise Exception(f"Signature generation failed: {response.text}")
